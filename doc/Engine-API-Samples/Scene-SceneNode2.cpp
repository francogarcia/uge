...
#include <Engine/GameLogic/SceneManager/SceneManager.h>
...
public:
    ...
    bool Init()
    {
        ...
        // GAME VIEW
        uge::XMLFile xmlFile;
        xmlFile.OpenFile("temp/graphics.xml", uge::File::FileMode::FileReadOnly);
        uge::GraphicalPreferences graphicalPreferences;
        graphicalPreferences.vInit(&xmlFile.GetRootElement());

        uge::IGraphicsSharedPointer pGraphics(LIB_NEW uge::OgreGraphics(L"Window Title", graphicalPreferences));
       
        const int TOTAL_BUFFERS = 4u;
        uge::IAudioSharedPointer pAudio(LIB_NEW uge::OpenALSoftAudio(TOTAL_BUFFERS));
        bool bSuccess = m_Output.Init(pGraphics,
                                      pAudio);
